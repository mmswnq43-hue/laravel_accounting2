...
<div class="page-header">
    <div>
        ...
        <p class="text-muted mt-2 mb-0">أنشئ حسابًا جديدًا مع أدوار جاهزة وصلاحيات تفصيلية تناسب مسؤولياته.</p>
    </div>
    <div>
                        ...
                        <div class="col-12 mt-4">
                            <div class="form-check form-switch card p-3 border-light-subtle">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <input class="form-check-input ms-0" type="checkbox" name="is_active" value="1" id="is_active" @checked(old('is_active', true))>
                                </div>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="form-check form-switch card p-3 border-light-subtle">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <input class="form-check-input ms-0" type="checkbox" name="must_change_password" value="1" id="must_change_password" @checked(old('must_change_password', true))>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-body p-4">
                    ...
                    <div class="d-flex flex-column gap-3">
                        @foreach ($roles as $role)
                            <div class="role-selection-item">
                                <input type="checkbox" class="btn-check" name="role_ids[]" id="role_{{ $role->id }}" value="{{ $role->id }}" @checked(in_array($role->id, old('role_ids', []), true))>
                    ...
                    <div class="mt-4 pt-4 border-top">
                        <button type="submit" class="btn btn-primary w-100 py-3 mb-2">
                            <i class="fas fa-user-check ms-1"></i> إنشاء المستخدم
                        </button>
                                    ...
                                    <div class="d-flex flex-column gap-2">
                                        @foreach ($groupPermissions as $permission)
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" name="permission_ids[]" value="{{ $permission->id }}" id="perm_{{ $permission->id }}" @checked(in_array($permission->id, old('permission_ids', []), true))>
