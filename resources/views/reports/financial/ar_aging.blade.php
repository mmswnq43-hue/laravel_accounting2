@extends('layouts.app')
@section('title', 'أعمار ديون العملاء')
@section('content')
<x-report-shell
    title="أعمار ديون العملاء"
    subtitle="تحليل الديون المستحقة على العملاء مصنّفة حسب مدة الاستحقاق"
    :export-route="route('reports.financial.ar-aging.export', request()->query())"
>
    ...
    <x-slot:kpis>
        ...
        <div class="col-md-3">
            <div class="rpt-stat-card">
                <div class="rpt-stat-icon red"><i class="fas fa-users"></i></div>
                <div class="rpt-stat-value rpt-negative">{{ number_format($totalAR, 2) }}</div>
                <div class="rpt-stat-label">إجمالي الذمم المدينة ({{ $company->currency }})</div>
            </div>
        </div>
        {{-- Current --}}
        <div class="col-md-3">
    ...
    {{-- ── Chart ── --}}
    <x-slot:chart>
        <h6 class="fw-bold mb-3">توزيع الذمم المدينة حسب الفئة الزمنية</h6>
        <canvas id="arAgingChart" height="80"></canvas>
    </x-slot:chart>
        ...
        <table class="table rpt-table mb-0">
            ...
            <tbody>
                ...
                @php
                    ...
                    $pct = $totalAR > 0 ? round(($bucketTotal / $totalAR) * 100, 1) : 0;
                @endphp
                <tr>
                    <td>
                ...
                <tr class="fw-bold bg-light">
                    ...
                    <td class="text-end rpt-negative">{{ number_format($totalAR, 2) }}</td>
    ...
    <div class="card-header bg-white border-top">
        <h6 class="mb-0 fw-bold">أعلى العملاء ديناً</h6>
    </div>
    <div class="table-responsive">
        <table class="table rpt-table mb-0">
            ...
            <tbody>
                @forelse($byCustomer as $i => $row)
                <tr>
                    <td class="text-muted small">{{ $i + 1 }}</td>
                    <td class="fw-semibold">{{ $row['customer']?->name ?? 'غير محدد' }}</td>
                    <td class="text-muted">{{ $row['customer']?->phone ?? '-' }}</td>
                    <td class="text-center">
